...
- "feathers" → "feather", "reflections" → "reflection", "edges" → "edge" (always singularize these)
- "warm and cold gold" → extract BOTH "warm gold" AND "cold gold" as separate color entities
...
2. Singularize nouns ("skeletons"→"skeleton", "black templars"→"black templar", "feathers"→"feather", "edges"→"edge", "reflections"→"reflection"). Exception: "tyranids" stays plural. Keep -ing endings ("feathering", "highlighting").
...
6. SPLIT COMPOUND COLOR PHRASES: When the transcript says "warm and cold gold", "red and orange tones", or similar conjunctions, extract each color separately (e.g., "warm gold" + "cold gold").
...
BEFORE RETURNING — review your list:
1. Remove anything that is not a painting technique, specific paint product, tool, miniature model, game term, or hobby concept. If in doubt, leave it out.
2. BRAND CHECK: Did you extract any Citadel paint (rhinox hide, mephiston red, abaddon black, mournfang brown, bugmans glow, death claw brown, etc.)? If yes, make sure "citadel" (type: paint_brand) is in your list. Did you extract any Vallejo paint? If yes, make sure "vallejo" (type: paint_brand) is in your list.
3. SINGULAR CHECK: Are any entity names plural (feathers, reflections, edges, skeletons)? Singularize them.
